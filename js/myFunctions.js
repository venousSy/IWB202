...
for (let i = 0; i < detailButtons.length; i++) {
  detailButtons[i].onclick = function () {
    ...
    detailButtons[
      i
    ].parentElement.parentElement.nextElementSibling.classList.toggle(
      "disable"
    );
  };
}
...
function checkInputFileds() {
  ...
  const nationalRe = /^(0[1-9]|[1][1-4])\d{9}$/;
  if (nationalNumber.value == "") {
    if (!nationalNumber.classList.contains("error")) {
      nationalNumber.classList.add("error");
    }
    nationalNumber.classList.add("error");
  } else if (!nationalRe.test(nationalNumber.value)) {
    if (!nationalNumber.classList.contains("error")) {
      nationalNumber.classList.add("error");
    }
  } else {
